//
//  FirebaseInstitutionSource.swift
//  CatLedger
//
...
/// Thin wrapper around the `institutions` Firestore collection. Holds no business logic, only
/// raw reads and writes — encoding/decoding and error mapping are the responsibility of the caller.
final class FirebaseInstitutionSource {

    private var collection: CollectionReference { Firestore.firestore().collection("institutions") }
    ...
    /// Fetches all documents whose `profileId` field matches the given value, ordered by name.
