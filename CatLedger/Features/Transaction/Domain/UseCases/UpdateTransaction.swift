...
final class UpdateTransaction {
    ...
    func execute(_ input: UpdateTransactionInput) async throws {
        ...
        guard Set(input.splits.map(\.accountId)).count == input.splits.count else {
        ...
        let updatedTransaction = Transaction(
            ...
            splits: input.splits,
            isChecked: currentTransaction.isChecked
        )
        try await repository.update(updatedTransaction)
    }
}
