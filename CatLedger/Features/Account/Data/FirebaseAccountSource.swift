...
/// Thin wrapper around the `accounts` Firestore collection. Holds no business logic, only
/// raw reads and writes — encoding/decoding and error mapping are the responsibility of the caller.
final class FirebaseAccountSource {

    private var collection: CollectionReference { Firestore.firestore().collection("accounts") }
    ...
    /// Fetches all documents whose `institutionId` field matches the given value, ordered by name.
    func fetchAll(institutionId: String) async throws -> [[String: Any]] {
        let snapshot = try await collection
            .whereField("institutionId", isEqualTo: institutionId)
            .order(by: "name")
            .getDocuments()
